...
for i in $(seq 1 30); do
    if curl -sf "${SCHEDULER}/" > /dev/null 2>&1; then
...
curl -sf "${SCHEDULER}/" > /dev/null 2>&1 || fail "Scheduler did not become ready"
...
if [ -n "${EXISTING_AGENT}" ]; then
    ...
    AGENT_TOKEN=$(post "/api/v1/tokens" '{"name":"compose-agents","role":"agent"}' \
        | jq -r '.token')
    ok "Agent token created: ${AGENT_TOKEN}"
    echo ""
    echo "  To use a least-privilege agent token instead of the admin token:"
    echo "  \$env:FORGE_API_TOKEN = '${AGENT_TOKEN}'"
fi
...
if curl -sf "${MINIO}/minio/health/live" > /dev/null 2>&1; then
    # Create the forge-artifacts bucket via S3 PUT bucket API.
    # Auth uses our minimal Sigv4 implementation baked into the forge binary —
    # but here we use mc-style curl with unsigned headers since MinIO allows it
    # when anonymous access is disabled but root credentials are used.
    curl -sf -X PUT "${MINIO}/forge-artifacts" \
        -H "x-amz-content-sha256: e3b0c44298fc1c149afbf4c8996fb92427ae41e4649b934ca495991b7852b855" \
        -H "Content-Length: 0" \
        2>/dev/null && ok "MinIO bucket 'forge-artifacts' ready" || \
        skip "MinIO bucket 'forge-artifacts' already exists or created"
else
