...
// TestArtifactUploadAndDownload verifies that a step can upload an artifact and a downstream step can download it.
...
func TestGeneratorStep(t *testing.T) {
	// Emit 3 child steps using a python3 one-liner
...
// TestOrgCreationAndPipelineSubmission verifies an org can be created and a pipeline submitted under it.
...
// testFlakyStepDetection verifies that steps with inconsistent pass/fail history
// are surfaced by the flaky detection endpoint.
...
// TestSecretScoping verifies that global, org-scoped, and project-scoped secrets can each be set and the system
// records the operation without error
