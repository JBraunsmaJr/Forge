...
// TestHealthCheck verifies the scheduler is reachable and the Web UI loads.
...
// TestSingleStepPipeline submits the simplest possible pipeline and verifies
...
// TestCancelPipeline verifies that a running pipeline can be canceled.
func TestCancelPipeline(t *testing.T) {
	// sleep 600 gives us a 10-minute window to cancel before it naturally finishes.
	...
	// Poll until the job is definitely "running" (up to 60 seconds).
	...
	// Verify the run reaches a terminal state.
	status := waitForRun(t, adminClient, runID)
