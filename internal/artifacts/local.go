// Package artifacts — local filesystem backend.
//
// Files are stored under cfg.LocalDir/{runID}/{artifactID}/{filename}.
// The scheduler serves them directly via its HTTP server.
// Pre-signed upload URLs include a one-time token stored in the DB.
package artifacts
...
func (s *LocalStore) PresignUpload(_ context.Context, req PresignRequest) (*PresignResponse, error) {
	...
	// One-time upload token — verified when the agent PUTs the file.
	b := make([]byte, 16)
	rand.Read(b)
...
func (s *LocalStore) DeleteArtifact(_ context.Context, artifactID string) error {
	...
	os.Remove(path)
	os.Remove(filepath.Dir(path)) // remove per-artifact dir if empty
	return nil
}
