// Package cache implements Forge's content-addressed store (CAS).
//
// # The core idea
//
// Every pipeline step gets a "task hash" — a SHA-256 digest computed from:
//   - The container image name
//   - The command and its arguments (sorted for determinism)
//   - The environment variables (sorted)
//   - The SHA-256 of every file matched by the step's `inputs` globs
//
// If we've seen that exact hash before and it passed, we skip execution
// entirely and replay the cached result. No re-running work that hasn't changed.
//
// # Why this is better than key strings
//
// GitHub Actions / GitLab CI make you write things like:
//
//	cache-key: ${{ runner.os }}-node-${{ hashFiles('**/package-lock.json') }}
//
// If you forget to include a file, the cache is wrong. If you include
// too many files, you get more misses than you should. And the cache only
// caches *downloaded dependencies*, not the step's actual *output*.
//
// Forge's CAS caches the step output keyed by a hash of its true inputs.
// You declare `inputs: ["src/**", "go.mod"]` and the system does the rest.
//
// # Storage layout (Phase 1: local disk)
//
//	.forge/cache/
//	  ab/
//	    ab3f9c2d1e...json    ← one JSON file per cached task
//	  f7/
//	    f7a021bc9d...json
//
// Using the first two hex chars as a subdirectory (called "sharding") avoids
// putting thousands of files in one flat directory — the same technique used
// by Git's object store.
package cache

import (
	"crypto/sha256"
	"encoding/hex"
	"encoding/json"
	"fmt"
	"io"
	"os"
	"path/filepath"
	"sort"
	"strings"
	"time"

	"github.com/JBraunsmaJr/forge/internal/pipeline"
)

// Entry is what gets stored in the CAS for a successful step execution.
// It records enough information to replay the result without re-running.
type Entry struct {
	TaskHash  string        `json:"task_hash"`
	StepID    string        `json:"step_id"`
	ExitCode  int           `json:"exit_code"`
	Duration  time.Duration `json:"duration_ns"`
	CreatedAt time.Time     `json:"created_at"`

	// Image and Command are stored for human-readable debugging —
	// so you can open the JSON file and understand what was cached.
	Image   string   `json:"image"`
	Command []string `json:"command"`
}

// Store is the content-addressed cache store.
// Phase 1: files on local disk.
// Phase 2: pluggable backend (local + optional shared remote).
type Store struct {
	dir string // root directory, typically .forge/cache
}

// New creates (or opens) a Store rooted at dir.
func New(dir string) (*Store, error) {
	if err := os.MkdirAll(dir, 0755); err != nil {
		return nil, fmt.Errorf("creating cache dir %s: %w", dir, err)
	}
	return &Store{dir: dir}, nil
}

// Lookup checks whether a cached result exists for taskHash.
//
// Returns (entry, true) on a hit, (nil, false) on a miss.
//
// Go's multiple return values mean we never need a sentinel value
// like null to distinguish "found but empty" from "not found".
// The (T, bool) pair is the idiomatic Go pattern for optional results —
// equivalent to returning Nullable<T> or (T?, bool) in C#.
func (s *Store) Lookup(taskHash string) (*Entry, bool) {
	path := s.entryPath(taskHash)

	data, err := os.ReadFile(path)
	if err != nil {
		// os.IsNotExist distinguishes "file not found" from other read errors.
		// Like catching FileNotFoundException specifically in C#.
		if os.IsNotExist(err) {
			return nil, false
		}
		// Treat unexpected read errors as a miss — don't crash the build
		// because the cache had a problem.
		return nil, false
	}

	var entry Entry
	if err := json.Unmarshal(data, &entry); err != nil {
		return nil, false
	}
	return &entry, true
}

// Store saves a cache entry for a completed step.
// Only called after a step passes (exit code 0) — we never cache failures.
func (s *Store) Store(entry *Entry) error {
	path := s.entryPath(entry.TaskHash)

	// Create the shard directory (e.g. .forge/cache/ab/) if needed.
	if err := os.MkdirAll(filepath.Dir(path), 0755); err != nil {
		return fmt.Errorf("creating shard dir: %w", err)
	}

	data, err := json.MarshalIndent(entry, "", "  ")
	if err != nil {
		return fmt.Errorf("marshaling cache entry: %w", err)
	}

	// Write to a temp file then rename — this makes the write atomic.
	// If the process is killed mid-write, we never leave a partial/corrupt
	// cache file. Rename is atomic on all POSIX systems.
	// (This is a common pattern you'll see throughout distributed systems code.)
	tmp := path + ".tmp"
	if err := os.WriteFile(tmp, data, 0644); err != nil {
		return fmt.Errorf("writing cache entry: %w", err)
	}
	if err := os.Rename(tmp, path); err != nil {
		return fmt.Errorf("committing cache entry: %w", err)
	}
	return nil
}

// entryPath returns the on-disk path for a given task hash.
// Uses the first 2 hex chars as a shard directory — the same technique as Git.
func (s *Store) entryPath(taskHash string) string {
	if len(taskHash) < 2 {
		return filepath.Join(s.dir, "misc", taskHash+".json")
	}
	return filepath.Join(s.dir, taskHash[:2], taskHash+".json")
}

// ComputeTaskHash calculates the content hash for a step.
//
// The hash is a SHA-256 over a deterministic serialization of:
//   - The container image
//   - The command (joined)
//   - The environment variables (sorted by key, then value)
//   - The SHA-256 of every file matched by the step's inputs globs
//
// Two steps with identical inputs will always produce the same hash.
// One changed file in the inputs will produce a completely different hash.
//
// workspaceDir is the project root — needed to resolve the inputs globs.
func ComputeTaskHash(step *pipeline.Step, workspaceDir string) (string, error) {
	// sha256.New() creates a hash.Hash — an io.Writer that accumulates
	// input and produces a digest. We write all our inputs into it in
	// a deterministic order, then call Sum(nil) at the end.
	//
	// C# equivalent: using var sha = SHA256.Create(); sha.ComputeHash(...)
	h := sha256.New()

	// Write image name.
	fmt.Fprintf(h, "image:%s\n", step.Image)

	// Write command (joined so ["sh", "-c", "echo hi"] and ["sh -c echo hi"]
	// don't accidentally collide).
	fmt.Fprintf(h, "command:%s\n", strings.Join(step.Command, "\x00"))

	// Write environment variables, sorted by key so map iteration order
	// (which is random in Go, like Dictionary in .NET) doesn't affect the hash.
	envKeys := make([]string, 0, len(step.Env))
	for k := range step.Env {
		envKeys = append(envKeys, k)
	}
	sort.Strings(envKeys)
	for _, k := range envKeys {
		fmt.Fprintf(h, "env:%s=%s\n", k, step.Env[k])
	}

	// Hash each input file.
	if err := hashInputFiles(h, step.Inputs, workspaceDir); err != nil {
		return "", fmt.Errorf("hashing inputs: %w", err)
	}

	// Produce the final hex digest.
	// hex.EncodeToString converts []byte to a hex string like "ab3f9c2d..."
	return hex.EncodeToString(h.Sum(nil)), nil
}

// hashInputFiles resolves glob patterns relative to workspaceDir,
// sorts the matched paths for determinism, then hashes each file's contents.
//
// h is the running SHA-256 hasher — we write into it rather than returning
// a separate hash, because we want all inputs to contribute to one digest.
func hashInputFiles(h io.Writer, globs []string, workspaceDir string) error {
	if len(globs) == 0 {
		// No inputs declared → step is never cached (always re-runs).
		// Signal this by writing a sentinel so the hash is always unique.
		fmt.Fprintf(h, "inputs:none\n")
		return nil
	}

	// Collect all matched paths.
	var paths []string
	for _, pattern := range globs {
		// filepath.Glob matches files on disk against a glob pattern.
		// It's like Directory.GetFiles(dir, pattern) in C#.
		matched, err := filepath.Glob(filepath.Join(workspaceDir, pattern))
		if err != nil {
			return fmt.Errorf("glob %q: %w", pattern, err)
		}
		paths = append(paths, matched...)
	}

	// Deduplicate (a file could match multiple globs).
	paths = dedup(paths)

	// Sort for determinism — glob returns results in filesystem order,
	// which varies by OS and filesystem.
	sort.Strings(paths)

	// Hash each file's path (relative to workspace) and content.
	for _, p := range paths {
		// Make the path relative so the hash is the same regardless of
		// where on disk the workspace lives.
		rel, err := filepath.Rel(workspaceDir, p)
		if err != nil {
			return err
		}

		f, err := os.Open(p)
		if err != nil {
			return fmt.Errorf("opening input file %s: %w", rel, err)
		}

		fileHash := sha256.New()
		if _, err := io.Copy(fileHash, f); err != nil {
			f.Close()
			return fmt.Errorf("hashing file %s: %w", rel, err)
		}
		f.Close()

		// Write "path:hash" into the master hasher.
		// Including the path means renaming a file (same content, new name)
		// correctly produces a different task hash.
		fmt.Fprintf(h, "file:%s:%s\n", rel, hex.EncodeToString(fileHash.Sum(nil)))
	}

	return nil
}

// dedup removes duplicate strings from a slice.
// Uses a map as a set — the idiomatic Go pattern for deduplication.
func dedup(ss []string) []string {
	seen := make(map[string]struct{}, len(ss))
	out := make([]string, 0, len(ss))
	for _, s := range ss {
		if _, exists := seen[s]; !exists {
			seen[s] = struct{}{}
			out = append(out, s)
		}
	}
	return out
}
