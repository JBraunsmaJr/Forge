...
func TestComputeTaskHash_NoInputs(t *testing.T) {
	...
	step := &pipeline.Step{
		Image:   "alpine:latest",
		Command: []string{"date"}, // always produces different output
	...
	// (there are no files to change between calls). The "never cache"
	// semantics are enforced at the executor level by checking len(step.Inputs).
	// This test just verifies the hash is stable for its inputs.
	if hash1 != hash2 {
...
func TestSharding(t *testing.T) {
	...
	// The file should be at <cacheDir>/ab/ab3f9c2d1e.json
