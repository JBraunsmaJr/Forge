// Package secrets provides a client for HashiCorp Vault's KV v2 secrets engine.
//
// # Secret scoping
//
// Secrets can be stored at three scopes, resolved in priority order:
//
//  1. Project scope  secret/data/forge/projects/{project_id}/{NAME}
//  2. Org scope      secret/data/forge/orgs/{org_id}/{NAME}
//  3. Global scope   secret/data/forge/global/{NAME}
//  4. Legacy         secret/data/forge/{NAME}          (backward compat)
//
// Use GetScoped when resolving secrets for a running job. Use Get/Set/List
// with an explicit ScopePath for the CLI commands.
//
// # Quick start
//
...
//	./forge.exe secret set MY_SECRET my-value --org <org-id>
package secrets
...
func (c *Client) GetScoped(name, orgID, projectID string) (string, error) {
	...
	// 4. Legacy path (backward compatibility with existing secrets)
