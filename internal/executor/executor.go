// Package executor runs pipeline steps inside Docker containers.
//
// Phase 2 adds CAS integration: before running a step, the executor
// computes its task hash and checks the cache. On a hit, execution is
// skipped entirely and the cached result is replayed.
package executor
...
func (e *Executor) RunStep(ctx context.Context, step *pipeline.Step) (*pipeline.StepResult, error) {
	...
	// ── Cache check ───────────────────────────────────────────────────
	// Only attempt a cache lookup if:
	//   1. A cache store is configured
	//   2. The step has inputs declared (no inputs = always re-run)
	//   3. It's a regular task (generators always run — their output is dynamic)
	if e.Cache != nil && len(step.Inputs) > 0 && step.Type != "generator" {
		taskHash, err := cache.ComputeTaskHash(step, e.WorkspaceDir)
		if err == nil {
			// Store the hash on the step so the result can reference it.
	...
	if err := cmd.Start(); err != nil {
		...
		// then return a failed result (not an error) so the log path is preserved.
...
func buildDockerArgs(step *pipeline.Step, workspaceDir string) []string {
	...
	// If FORGE_DOCKER_NETWORK is set, join that network.
	// This is needed for job containers to reach the scheduler in Compose environments.
	if net := os.Getenv("FORGE_DOCKER_NETWORK"); net != "" {
		args = append(args, "--network", net)
	}

	/*
		docker_socket: true mounts the host Docker daemon socket, so this
		step can run docker build / docker run itself (Docker-outside-Docker pattern).
		Requires docker-cli to be present in the step's image
	*/
