// Package store handles database connectivity and schema migration.
//
// # Setup
//
...
//	  postgres:16-alpine
//
//	$env:FORGE_DB_URL = "postgres://postgres:forge@localhost:5432/forge?sslmode=disable"
//
// # Why Postgres for a job queue
//
// The most important query in the whole system is LeaseNext:
//
//	SELECT id FROM jobs WHERE status='queued'
//	LIMIT 1 FOR UPDATE SKIP LOCKED
//
// SKIP LOCKED means: "give me a row that nobody else has locked right now."
// Two agents calling this simultaneously each get a DIFFERENT job — no double
// assignment, no distributed lock manager needed. This is the same technique
// used by Sidekiq (Ruby), Celery (Python), and Que (Go/Postgres).
//
// A Redis-based queue or a dedicated broker (RabbitMQ, NATS) would also work,
// but Postgres's SKIP LOCKED gives us exactly this with no extra infrastructure.
package store
...
// Open opens a Postgres connection and runs schema migrations.
...
func Open(connStr string) (*sql.DB, error) {
	...
	// For a learning project, "CREATE TABLE IF NOT EXISTS" is fine.
	// Production systems use a migration tool (golang-migrate, goose, atlas).
...
//
//  1. orgs          (no deps)
//  2. api_tokens    (no deps)
//  3. runs          (→ orgs)
//  4. jobs          (→ runs)
//  5. job_logs      (→ jobs)
//  6. policies      (→ orgs)
//  7. projects      (→ orgs)
//  8. artifacts     (→ runs, jobs)
