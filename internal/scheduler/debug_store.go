// Package scheduler — debug session store.
//
// A debug session is an idle Docker container (kept alive with tail -f /dev/null)
// that the user can exec commands into from the web UI. It gives them a shell
// in the exact environment a failed job ran in: same image, same env vars,
// same workspace mount.
//
// # Lifecycle
//
//	CreateSession() → status: "starting"
//	     ↓ (agent picks it up and starts the container)
//	RegisterContainer() → status: "ready"
//	     ↓ (user runs commands via ExecCommand / PollCommands / SubmitOutput)
//	CloseSession() → status: "closed"  (or auto-close after TTL)
package scheduler
...
// CreateSession opens a new debug session for a failed job.
// The caller provides the job's spec (image, env, workspace) so the
// agent can recreate the exact environment.
...
// Also touches the session TTL — activity keeps the session alive.
func (d *DebugStore) ExecCommand(sessionID, input string) (string, error) {
	...
	// Reset the TTL on every command — the session lives as long as the
...
func (d *DebugStore) PollCommands(sessionID string) (*api.PollCommandsResponse, error) {
	...
	s.cancelRequested = false // agent clears it after seeing it
...
// CheckAndClearCancel returns true if a cancel was requested and clears the flag.
// Intentionally does NOT touch pendingCmds — the cancel goroutine uses this
// so it doesn't accidentally drain commands meant for the main execute loop.
func (d *DebugStore) CheckAndClearCancel(sessionID string) (bool, bool) {
	...
	s, ok := d.sessions[sessionID]
	if !ok {
		return false, false // second bool = session exists
	}
	cancel := s.cancelRequested
...
func (d *DebugStore) SubmitOutput(sessionID string, out api.DebugOutput) error {
	...
	// (Keeps the store package import-light.)
	evt := fmt.Sprintf(
...
// CloseSession shuts down a session. The agent will detect this on its next poll
...
// broadcast sends an event to all SSE subscribers. Caller must hold mu.
