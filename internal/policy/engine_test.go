...
// TestApply_InlineScriptTransformer tests the transformer path using an
// inline shell script that reads the pipeline and adds a step.
...
func TestApply_InlineScriptTransformer(t *testing.T) {
	...
	// Script: reads stdin, adds "trivy-scan" after "build" and makes
	// deploy depend on trivy-scan instead of build.
	script := `
input=$(cat)
...
`
	...
	// Should have 3 steps: build, trivy-scan, deploy
	if len(result) != 3 {
...
// TestApply_TransformerPassThrough verifies that a transformer that echoes
// the pipeline unchanged produces identical output.
func TestApply_TransformerPassThrough(t *testing.T) {
	userSteps := []api.StepDef{{ID: "lint"}, {ID: "test"}}
	pol := api.PolicyInfo{
		Name: "passthrough",
		Transformer: &api.PolicyTransformer{
			// Read stdin, output .steps unchanged
	...
	// Even if script falls back to [], we just verify no crash.
	_ = result
}

// TestApply_StaticAndTransformerCompose verifies that static injection and
...
func TestApply_StaticAndTransformerCompose(t *testing.T) {
	...
	// Policy has both static steps AND a transformer.
	// Transformer should see the statically injected step.
	pol := api.PolicyInfo{
		Name:  "composed",
		Steps: []api.StepDef{{ID: "static-scan", Image: "alpine:latest"}},
		Transformer: &api.PolicyTransformer{
			// Returns all steps plus one more
			Script: `python3 -c "
import json, sys
...
" 2>/dev/null || python3 -c "import json,sys; print(json.dumps(json.load(sys.stdin)['steps']))"`,
	...
	// Should have: static-scan (injected first) + lint (user) + transformer-step (added)
	if len(result) < 2 {
...
// compile-time check that TransformerInput serialises correctly
