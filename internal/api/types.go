// Package api defines the shared types used for HTTP communication
// between the Forge scheduler and its agents.
//
// Keeping these in a dedicated package means both the scheduler (server)
// and the agent (client) import from the same source of truth.
// No type drift, no manual JSON field name matching.
//
// C# equivalent: a shared DTO project referenced by both
// your API project and your client project.
package api
...
// ── Request / Response types (one per endpoint) ───────────────────────────────
...
type CreateProjectRequest struct {
	Name         string `json:"name"`
	RepoURL      string `json:"repo_url"`
	PipelinePath string `json:"pipeline_path,omitempty"` // defaults to .forge/pipeline.json
	SCMToken     string `json:"scm_token,omitempty"`     // for private repos
}
